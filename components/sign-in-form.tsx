...
export function SignInForm() {
  const [fields, setFields] = useState<Fields>({ fullName: "", username: "", email: "", password: "" })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!fields.username.trim()) { setError("Username is required"); return }
    ...
    try {
      ...
      if (!res.ok) {
        setError(await readApiError(res, "Sign up failed"))
        setLoading(false)
        return
      }
  ...
  return (
    <div className="flex w-full max-w-sm flex-col gap-4">
      <div className="flex flex-col items-center rounded-2xl border border-border bg-card/90 px-10 py-9 shadow-xl shadow-foreground/5 backdrop-blur-sm">
        <h1 className="brand-text mb-1 font-serif text-5xl  font-extrabold tracking-tight">K-Verse</h1>
        <p className="mb-6 text-center text-sm font-medium leading-relaxed text-muted-foreground">
          Sign up to see photos and videos from your friends.
        </p>

        <form onSubmit={handleSubmit} className="flex w-full flex-col gap-2">
          <FloatingInput label="Full name" name="fullName" type="text" autoComplete="name" value={fields.fullName} onChange={handleChange} />
          <FloatingInput label="Username" name="username" type="text" autoComplete="username" value={fields.username} onChange={handleChange} />
          <FloatingInput label="Email" name="email" type="email" autoComplete="email" value={fields.email} onChange={handleChange} />
      ...
      <div className="rounded-2xl border border-border bg-card/90 px-10 py-5 text-center shadow-lg shadow-foreground/5 backdrop-blur-sm">
        <p className="text-sm text-foreground">
          Have an account?{" "}
          <Link href="/login" className="brand-text font-semibold hover:opacity-80">Log in</Link>
        </p>
      </div>
    </div>
  )
